...
function Compute_Corrections!(vert_coord::Vert_Coordinate, mesh::Spectral_Spherical_Mesh, atmo_data::Atmo_Data,
  ...
  grid_t_n::Array{Float64, 3}, spe_t_n::Array{ComplexF64, 3}, grid_tracers_p::Array{Float64, 3}, grid_tracers_c::Array{Float64, 3}, grid_tracers_n::Array{Float64, 3},grid_δtracers::Array{Float64,3}, grid_tracers_all::Array{Float64,3},sum_tracers_p::Float64, spe_tracers_n::Array{ComplexF64, 3},  save_file_name::String= "None")
  ...
  if (do_water_correction) 
    ...
    #sum_tracers_n    = Area_Weighted_Global_Mean(mesh, grid_tracers_all)
        ...
        for j in collect(1:nθ)
            for i in collect(1:nλ)
                #grid_tracers_n[i,j,k] += (grid_tracers_n[i,j,k]/sum_tracers_n) *  tracers_correction
                 grid_tracers_n[i,j,k] *= tracers_ratio
    ...
    #    @save save_file_name sum_tracers_p sum_tracers_n 
    #end
       # mean_tracers_p mean_tracers_n
    #print(sum(total))
    #end
...
function Spectral_Dynamics!(mesh::Spectral_Spherical_Mesh,  vert_coord::Vert_Coordinate, 
  ...
  *** The advecting velocities are taken from the centered time t.        ***
  ...
  grid_energy_full,
  grid_ps_n, spe_lnps_n, 
  grid_t_n, spe_t_n, grid_tracers_p, grid_tracers_c, grid_tracers_n, grid_δtracers, grid_tracers_full, sum_tracers_p, spe_tracers_n, "0511_300day_100_sumtracers_Spectral_Dynamic.dat")
